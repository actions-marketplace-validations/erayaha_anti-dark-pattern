...
exports.GitHubModelsPromptModel = GitHubModelsPromptModel;
...
function normalizeModelFindings(rawContent, input) {
    ...
    return findings.flatMap((finding) => {
        ...
        const indexedPosition = typeof evidence === 'string' ? resolvePositionFromEvidence(input.content, evidence) : undefined;
        ...
        return [
            {
                ...
                line,
                column,
                message: firstNonEmptyString(finding.message) ??
...
function parseModelFindings(rawContent) {
    ...
    try {
        ...
        throw new Error(`GitHub Models returned invalid JSON findings: ${error instanceof Error ? error.message : String(error)}`);
    }
...
function resolvePositionFromEvidence(content, evidence) {
    const matchIndex = content.indexOf(evidence);
    if (matchIndex < 0) {
        return undefined;
    }
    return (0, utils_1.findLineAndColumn)(content, matchIndex);
}
